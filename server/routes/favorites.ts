...
router.get('/:clerkUserId', async (req: Request, res: Response) => {
  ...
  try {
    ...
    return res.json(favorites.map(fav => ({ propertyId: fav.propertyId._id })))
  } catch (error) {
...
router.delete('/:clerkUserId/:propertyId', async (req: Request, res: Response) => {
  const { clerkUserId, propertyId } = req.params
  try {
    const user = await User.findOne({ clerkUserId })
    if (!user) return res.status(404).json({ message: 'User not found' })

    const result = await Favorite.findOneAndDelete({ userId: user._id, propertyId })
    if (!result) return res.status(404).json({ message: 'Favorite not found' })

    return res.status(200).json({ message: 'Favorite removed successfully' })
  } catch (error) {
    console.error('Error removing favorite:', error)
    return res.status(500).json({ message: 'Failed to remove favorite', error })
  }
})
